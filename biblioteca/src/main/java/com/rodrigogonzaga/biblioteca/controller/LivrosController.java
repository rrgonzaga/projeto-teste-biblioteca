...
public class LivrosController {
	...
	public ModelAndView novo(Livro livro) {		 
		...
		mv.addObject("autores", autores.findAllByOrderByNomeAsc());		
	...
	public ModelAndView cadastrar(@Valid Livro livro, BindingResult result, Model model, RedirectAttributes attributes) {
		...
		catch(IsbnLivroJaCadastradoException ex) {
			result.rejectValue("isbn", ex.getMessage(), ex.getMessage());
			livro.setAutor(null);
			return novo(livro);
		}
		catch(TituloLivroJaCadastradoException ex) {
			result.rejectValue("titulo", ex.getMessage(), ex.getMessage());
			livro.setAutor(null);
			return novo(livro);
		}		
